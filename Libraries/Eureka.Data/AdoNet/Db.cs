    ...
    public class Db
    {
        ...
        public Db(string conn = null)
        {
            if (conn == null) // index is 1 because 0 = localdb
                connectionString = ConfigurationManager.ConnectionStrings["ms_sql"].ConnectionString;
            else
        ...
        DbConnection CreateConnection()
        {
            ...
            try
            {
                ...
                Session.Instance.DatabaseName = connection.Database;
                Session.Instance.IPAddress = connection.DataSource;
                Session.Instance.StatusConnected = string.Format("{0} / ConnectionTimeout: {1}", connection.State, connection.ConnectionTimeout);
                return connection;
            }
            catch
            {
                Session.Instance.DatabaseName = string.Empty;
                Session.Instance.IPAddress = string.Empty;
                Session.Instance.StatusConnected = "Not Connected";
                return null;
            }
        }

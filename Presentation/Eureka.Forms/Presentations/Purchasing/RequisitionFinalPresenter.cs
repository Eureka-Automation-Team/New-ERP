...
{
    public class RequisitionFinalPresenter
    {
        private readonly IRequisitionFinalView _view;
        ...
        private void Sorting(object sender, EventArgs e)
        {
            ...
            if (lines.Count > 0)
            {
                if (!string.IsNullOrEmpty(_view.SortBy))
                {
                    ...
                    if (_view.SortBy == "Supplier")
                    {
                        if (_view.SortType == "Z-A")
                            lines = lines.OrderByDescending(o => o.SuplierSymbol).ToList();
                        else
                            lines = lines.OrderBy(o => o.SuplierSymbol).ToList();
                    }
        ...
        private void ConvertPO_Click(object sender, EventArgs e)
        {
            ...
            {
                if(resVendors.FirstOrDefault().Vendor == 0)
                {
                    MessageBox.Show("Vendor is null!, cannot convert PO to one by one.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                else
                {
            ...
            else
            {
                ...
                vendor = _repoPay.GetVendorByID(vendorId);
            }
            ...
            var approver = _repoUser.GetByID(head.BuyerId);
            if (approver != null)
            {
                head.ApproverId = approver.ApproverId;
            }
            ...
            else if (head.TypeLookupCode == "MAKING")
            {
                if (head.JobFlag)
                {
                    DialogResult dialogResult = MessageBox.Show(string.Format("This PO will be generate a Job Process type. Are you sure to confrim?{0} Yes = Job Process type.{0} No = PO Standard type."
                                                                , Environment.NewLine)
                                                                , "Please confirm."
                ...
                try
                {
                    if (_view.linesSelected != null)
                    {
                        ...
                        foreach (RequisitionLineModel item in _view.linesSelected)
                        {
                            ...
                            pol.EncumbranceFlag = item.EncumbranceFlag;
                            pol.EncumbranceAmount = item.EncumbranceAmount;
        ...
        private void Save_Changed(object sender, EventArgs e)
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (budgetCost.UfBudgetRemain >= line.ExtendedAmount)
                {
                    ...
                    //Add Cost amount usage by PR Line.
                    //budgetCost.FcstCostAmount = budgetCost.FcstCostAmount + line.ExtendedAmount;
            ...
            var grpHeads = lines.GroupBy(item => item.RequisitionHeaderId)
                      .Select(group => new { reqHeaderId = group.Key, ReqLines = group.ToList() })
                      .ToList();
            foreach (var req in grpHeads)
            {
                _repository.UpdateCancelFlag(req.reqHeaderId);
            }
        ...
        private void Filter(object sender, EventArgs e)
        {
            if(_view.projectParam != null)
            {
                List<RequisitionLineModel> lines = _repository.GetLinesByProjectID(_view.projectParam.Id)
                ...
                if(!string.IsNullOrEmpty(_view.vendorName)) lines = lines.Where(x => (string.IsNullOrEmpty(x.VendorName) ? "" : x.VendorName.ToUpper()).Equals(_view.vendorName.ToUpper())).ToList();
                if (!_view.CheckAll)
                {
                    if (_view.pendingParam) lines = lines.Where(x => !x.FinalConfirmFlag && !x.RejectFlag).ToList();
                    if (_view.rejectParam) lines = lines.Where(x => x.RejectFlag).ToList();
                    if (_view.confirmedParam) lines = lines.Where(x => x.FinalConfirmFlag).ToList();
                }
